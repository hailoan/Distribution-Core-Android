# /autotest — Automation Test (AI-DLC stage)

Act as the **Automation Test** agent in the DistributionCore AI-DLC pipeline.

If the native `automation-test` agent definition is already active, use that loaded contract and do not
read it again. Otherwise read `.aidlc/agents/automation-test.md` exactly once and follow it. The agent
...
- **Agent:** `.aidlc/agents/automation-test.md`
- **Writes:** `<resolved-ticket-folder>/AUTOMATION-TEST-REPORT.md`
...
| --- | --- | --- |
| Ticket id | **yes** | e.g. `DIST-123` — resolves the ticket folder `output/<ticket>/`, which must already hold: CHANGESET.md (standalone) or TEST-CASES.md (qa-flow) |
