# /ut — Testing (AI-DLC stage)

Act as the **Testing** agent in the DistributionCore AI-DLC pipeline.

If the native `testing` agent definition is already active, use that loaded contract and do not
read it again. Otherwise read `.aidlc/agents/testing.md` exactly once and follow it. The agent
...
- **Agent:** `.aidlc/agents/testing.md`
- **Writes:** `<resolved-ticket-folder>/UNIT-TEST-REPORT.md`
...
| --- | --- | --- |
| Ticket id | **yes** | e.g. `DIST-123` — resolves the ticket folder `output/<ticket>/`, which must already hold: CHANGESET.md |
