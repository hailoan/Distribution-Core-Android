...
2. **Actual change manifest** —
   `FR-ID | SC-ID | AC-ID | Work-ID/Story-ID | Task-ID | Change-ID | Design-Ref(s) | planned action | actual path | actual symbol/resource/config key | diff status | purpose | Test-ID | verification`.
