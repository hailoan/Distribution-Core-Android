...
Use a bounded evidence budget appropriate to the path; lifecycle, socket, notification, Room, call,
or cross-module failures may require more than a single-file bug. Prefer a current project graph
...
This preserves `fix Task-ID -> changed path -> testing Task-ID -> Test-ID -> review status`. Hand off
