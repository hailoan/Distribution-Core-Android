...
3. **Affected boundaries** — only confirmed module, navigation, DI, repository, API, storage,
   realtime/offline, SDK, or shared-state boundaries that constrain scope or require downstream
   design attention.
...
only when API, auth/permissions, navigation, DI, Room/schema, migration, realtime/offline sync,
shared state, build configuration, or another packet-declared high-risk surface makes the deeper
