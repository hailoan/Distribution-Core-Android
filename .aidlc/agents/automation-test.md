...
- **automation-test-flow** → `CHANGESET.md` (test already-implemented changes on demand).
- **qa-flow** → `TEST-CASES.md` (automate the pre-written, automatable test cases).

Use `$AIDLC_FLOW` when set; otherwise resolve `qa-flow` when `TEST-CASES.md` exists and
`automation-test-flow` when a `CHANGESET.md` exists.
Then run:
...
the handoff — the CHANGESET Testing Handoff (standalone) or the TEST-CASES Automation Handoff
(qa-flow); report missing ownership/trace as a gap rather than inventing it.

### Scope boundary with the testing stage

Load `module-impact-analysis.md` and place coverage in the owning module's existing
`src/androidTest` root; do not default to `app`. This stage owns **instrumented / UI** coverage — Espresso view
...
- **standalone flow** — the CHANGESET Testing Handoff. Preserve its Test-ID, AC/risk, level,
