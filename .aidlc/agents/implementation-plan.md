...
Load `module-impact-analysis.md` first and reconcile the design against current Gradle/source
edges. Load `planning.md` for vertical work items, executable tasks, the DAG, and waves. Load its optional
...
Load `native-boundary-guideline.md` only for planned camera/JNI/C++/CMake/ABI work — adding
`ffmpeg-guideline.md` for planned `AVFrame`/swscale/FFmpeg-packaging work, `opengles-guideline.md`
for planned EGL/GLES/GLSL/FBO work, or `ndk-cpp-guideline.md` for planned JNI/C++/toolchain work —
and `gradle-module-guideline.md` only for planned module/build/plugin/publication work; convert
their required checks into integration-testing tasks rather than leaving them as prose.
...
4. **Task backlog** —
   `Task-ID | Work-ID/Story-ID | owning module | owner stage (android-dev/testing/integration-testing) | objective | Change-IDs/exact path-symbol scope | Design-Ref(s) | preconditions/inputs | invariants | done condition | verification/Test-ID/Check-ID | depends on | collision key`.
...
7. **Test scope and verification matrix** —
   ...
   Follow it with the **module integration matrix**:
